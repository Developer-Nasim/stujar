...
class EventController extends Controller
{
    ...
    {
        $events = Event::select('id','title','logo','banner','status','event_type')
        ->orderby('id','desc')
        ->where('status',1)
        ->orWhere('status',2)
        ->get();
    ...
    public function store(Request $request)
    {
        ...
        if(!empty($request->file)){
            ...
            $isImageName = Event::where('file', 'LIKE', "%{$image_full_name}%")->get()->count();
            // dd($isImageName);
            if($isImageName > 0){
    ...
    public function edit(string $id)
    {
        ...
        return view('event.edit',compact('event','websettings'));
    }
